...
public class DataScopeAOP {
    ...
    @Pointcut("@annotation(com.mz.common.annotation.DataScope)")
    ...
    private void handleDeptDataScope(JoinPoint jp, String alias){
        TSysUser user = (TSysUser)SecurityUtils.getSubject().getPrincipal();
        AdminEntity entity = (AdminEntity)jp.getArgs()[0];
        ...
        if(entity instanceof TSysRole){
            sql = String.format("and %sid in (select rp2.role_id\n" +
                    ...
                    "where ur.user_id='%s' and rp.role_id=ur.role_id and p.id=rp.perm_id\n" +
            ...
            sql = String.format("and %s%s in (select id from (select distinct d.id\n" +
                    ...
                    "where ur.user_id='%s' and rp.role_id=ur.role_id and p.id=rp.perm_id and FIND_IN_SET(p.resource_id,concat(d.id,',',d.pids))\n" +
    ...
    private void handleMenuDataScope(JoinPoint jp){
        ...
        String sql = String.format(
                    "and m.id in (" +
                        ...
                        "and ur.user_id='%s' and p.perm_type='MENU')",user.getId());
